...
publish_one() {
    ...
    payload=$(printf '{"id":"%s","raw_payload":{"last_heartbeat_at":"%s","error_code":%s,"occurred_at":"%s","ingested_at":"%s","idempotency_key":"%s","status":"%s","kmp":"KMP-%s","scan_rate_pts_per_sec":%s,"point_cloud_quality_score":%s}}' \
