...
& $engine exec tsdb-lidar-pg psql -U postgres -d lidar -X -q -tAc "SELECT '  db last     : ' || COALESCE(max(kafka_offset)::text, '-') || '  (' || count(*) || ' records)' FROM rdb.lidar_status_message"

Write-Host "── TimescaleDB ───────────────────────────────────"
$sql = @'
SELECT 'lidar_status rows (approx)'        AS item, approximate_row_count('tsdb.lidar_status')::text        AS value
UNION ALL SELECT 'lidar_scan_actual rows',   approximate_row_count('tsdb.lidar_scan_actual')::text
UNION ALL SELECT 'lidar_scan_artifact rows', approximate_row_count('tsdb.lidar_scan_artifact')::text
...
UNION ALL SELECT 'hypertable size',
       pg_size_pretty(hypertable_size('tsdb.lidar_status') + hypertable_size('tsdb.lidar_scan_actual')
                      + hypertable_size('tsdb.lidar_scan_artifact'))
UNION ALL SELECT 'newest event', (SELECT max(last_event_at)::text FROM rdb.lidar_device_state)
UNION ALL SELECT 'freshness',    (SELECT (now() - max(last_event_at))::text FROM rdb.lidar_device_state)
UNION ALL SELECT 'devices',      (SELECT count(*)::text FROM rdb.lidar_device_state)
UNION ALL SELECT 'rejects',      (SELECT count(*)::text FROM rdb.lidar_ingest_reject)
UNION ALL SELECT 'tag catalog',  (SELECT count(*)::text FROM rdb.lidar_tag_catalog)
-- 카탈로그가 못 푼 장비는 tid 가 '#숫자' 로 남는다. 0 이 아니면 등록부를 다시 내보내야 한다.
UNION ALL SELECT 'unresolved devices',
       (SELECT count(*)::text FROM rdb.lidar_device_state WHERE tid LIKE '#%');
...
FROM rdb.lidar_status_message GROUP BY send_topic ORDER BY send_topic;

SELECT status, count(*) AS devices FROM rdb.lidar_device_state GROUP BY status ORDER BY status;
SELECT error_code, count(*) AS devices FROM rdb.lidar_device_state WHERE error_code IS NOT NULL GROUP BY 1 ORDER BY 2 DESC;

SELECT artifact_type, count(*) AS rows_5m, count(DISTINCT scan_id) AS scans,
       pg_size_pretty(sum(file_size_bytes)) AS bytes
FROM tsdb.lidar_scan_artifact WHERE time > now() - INTERVAL '5 minutes'
