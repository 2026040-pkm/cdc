...
# 원천은 lidar 스택(timescaledb)의 TimescaleDB 다. 그쪽 네트워크(tsdb-net)가 없으면
...
Write-Host ""
Write-Host "── 기동 완료 ─────────────────────────────────────"
Write-Host "  source DB   : localhost:59432 (lidar / postgres:postgres) — timescaledb 스택"
Write-Host "  target DB   : localhost:60433 (targetdb / postgres:postgres) - TimescaleDB 2.29 / PG17"
Write-Host "  cdc-service : http://localhost:60080/actuator/health"
Write-Host "  Prometheus  : http://localhost:60090"
Write-Host "  Grafana     : http://localhost:60300  (admin/admin)"
