...
/**
 * lidar 스택(src/timescaledb) 표 세 개의 핸들러. 컬럼 목록이 곧 설정이다.
 *
...
public class LidarPassthroughHandlers {
    ...
    TableSyncHandler lidarDeviceStateSyncHandler(PassthroughRepository repository) {
        return new PassthroughSyncHandler(
                ...
                List.of("tid"),
                List.of("tid", "status", "error_code", "scan_rate_pts_per_sec", "point_cloud_quality_score",
                        "temperature_c", "connectivity_rssi", "last_event_at", "last_heartbeat_at", "updated_at"),
                repository);
    }
    ...
    TableSyncHandler lidarStatusMessageSyncHandler(PassthroughRepository repository) {
        return new PassthroughSyncHandler(
                ...
                List.of("kafka_partition", "kafka_offset", "kafka_ts", "uniqueid", "msg_ts", "message_version",
                        "method_id", "item_count", "reject_count", "received_at"),
                repository);
    }
