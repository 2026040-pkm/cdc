 ...
 * 기동 중인 emb-cdc-tsdb-service 의 publication·슬롯과 겹치면 서로의 결과를 오염시킨다.
 */
public abstract class VerificationSupport {
    ...
    /**
     * 기동 중인 emb-cdc-tsdb-service 를 전제로 하는 시나리오의 사전 확인.
     *
    ...
    protected static void requireServiceRunning() {
        ...
        if (!up) {
            throw new IllegalStateException(
                    "emb-cdc-tsdb-service 가 기동 중이어야 한다 (" + HEALTH_URL + "). 먼저 ./scripts/up.sh 를 실행할 것");
        }
    }
    ...
    protected static final String HEALTH_URL =
            System.getProperty("cdc.verify.health.url", "http://localhost:60080/actuator/health");
