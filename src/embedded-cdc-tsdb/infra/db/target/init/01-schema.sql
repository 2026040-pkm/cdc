-- ============================================================================
-- Target DB 스키마 (PostgreSQL 17 + TimescaleDB)
--
-- 표는 embedded-cdc 의 target 과 한 줄씩 같다. 엔진만 TimescaleDB 다 —
-- 하이퍼테이블로 만드는 표는 없다. 원천이 CDC 로 흘려보내는 것이 전부 일반 표이기 때문이다
-- (하이퍼테이블 셋은 publication 에 없다. 02-lidar.sql 주석 참고).
-- 확장을 켜 두는 것은 수신 측에서 청크·압축을 붙여 보는 실험을 여기서 바로 할 수 있게
-- 하기 위해서다. Grafana 의 Target 데이터소스도 timescaledb: true 로 잡혀 있다.
-- ============================================================================
