...
CALL remove_columnstore_policy('tsdb.lidar_status', if_exists => true);
CALL add_columnstore_policy('tsdb.lidar_status', after => INTERVAL '3 days');

CALL remove_columnstore_policy('tsdb.lidar_scan_actual', if_exists => true);
CALL add_columnstore_policy('tsdb.lidar_scan_actual', after => INTERVAL '3 days');

CALL remove_columnstore_policy('tsdb.lidar_scan_artifact', if_exists => true);
CALL add_columnstore_policy('tsdb.lidar_scan_artifact', after => INTERVAL '3 days');
...
SELECT add_retention_policy('tsdb.lidar_status',        drop_after => INTERVAL '31 days');
SELECT add_retention_policy('tsdb.lidar_scan_actual',   drop_after => INTERVAL '31 days');
SELECT add_retention_policy('tsdb.lidar_scan_artifact', drop_after => INTERVAL '31 days');
...
CREATE MATERIALIZED VIEW tsdb.lidar_status_1m
...
SELECT add_continuous_aggregate_policy('tsdb.lidar_error_1m',
    start_offset      => INTERVAL '1 hour',
    end_offset        => INTERVAL '2 minutes',
    schedule_interval => INTERVAL '1 minute');
...
CREATE MATERIALIZED VIEW tsdb.lidar_scan_1m
WITH (timescaledb.continuous, timescaledb.materialized_only = false) AS
SELECT time_bucket(INTERVAL '1 minute', time) AS bucket,
       tid,
       stage,
...
FROM tsdb.lidar_scan_actual
GROUP BY bucket, tid, stage
WITH NO DATA;

SELECT add_continuous_aggregate_policy('tsdb.lidar_scan_1m',
    start_offset      => INTERVAL '1 hour',
    end_offset        => INTERVAL '2 minutes',
    schedule_interval => INTERVAL '1 minute');

-- 6) 산출물 1분 집계. 종류별 건수와 바이트 — NAS 에 하루 얼마가 쌓이는지가 여기서 나온다.
CREATE MATERIALIZED VIEW tsdb.lidar_artifact_1m
WITH (timescaledb.continuous, timescaledb.materialized_only = false) AS
SELECT time_bucket(INTERVAL '1 minute', time) AS bucket,
       artifact_type,
       count(*)                       AS cnt,
...
FROM tsdb.lidar_scan_artifact
GROUP BY bucket, artifact_type
WITH NO DATA;

SELECT add_continuous_aggregate_policy('tsdb.lidar_artifact_1m',
    start_offset      => INTERVAL '1 hour',
    end_offset        => INTERVAL '2 minutes',
    schedule_interval => INTERVAL '1 minute');

-- 7) 집계는 원본보다 오래 남긴다. 원본 31일 < 집계 1년.
SELECT add_retention_policy('tsdb.lidar_status_1m',   drop_after => INTERVAL '1 year');
SELECT add_retention_policy('tsdb.lidar_error_1m',    drop_after => INTERVAL '1 year');
SELECT add_retention_policy('tsdb.lidar_scan_1m',     drop_after => INTERVAL '1 year');
SELECT add_retention_policy('tsdb.lidar_artifact_1m', drop_after => INTERVAL '1 year');
